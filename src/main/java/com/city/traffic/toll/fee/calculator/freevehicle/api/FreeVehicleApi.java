...
public class FreeVehicleApi {
    ...
    @PostMapping
    public ApiResponse<FreeVehicleResponse> add(@RequestHeader("email") String email, @Valid @RequestBody FreeVehiclePayload freeVehiclePayload){
        return freeDayService.add(email, freeVehiclePayload);
    }

    @PutMapping("/{freeVehicleId}")
    public ApiResponse<FreeVehicleResponse> update(@RequestHeader("email") String email, @Valid @PathVariable("freeVehicleId") Long id, @RequestBody FreeVehiclePayload freeVehiclePayload){
